...
export class Stake extends SolanaManager {
    ...
    async create(address, amount, unstakeDays) {
        ...
        try {
            ...
            const [vault] = await PublicKey.findProgramAddress([utf8.encode('vault'), mint.toBuffer(), address.toBuffer()], new PublicKey(this.config.stake_address));
            const [stake] = await PublicKey.findProgramAddress([utf8.encode('stake'), mint.toBuffer(), address.toBuffer()], new PublicKey(this.config.stake_address));
    ...
    async topup(stakeAmount) {
        try {
            ...
            if (this.stakeAccounts && this.poolAccounts) {
                ...
                const response = await this.stake.program?.methods.topup(new anchor.BN(stakeAmount))
                    .accounts(this.stakeAccounts)
                    .preInstructions(preInstructions)
                    .postInstructions([
    ...
    async extend(stakeDurationSeconds) {
        ...
        if (this.stakeAccounts && this.poolAccounts) {
            try {
                ...
                const response = await this.stake.program?.methods.extend(new anchor.BN(stakeDurationSeconds))
                    .accounts(this.stakeAccounts)
                    .preInstructions([
                    ...preInstructions,
