...
import { getAssociatedTokenAddress, getAccount, ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, createAssociatedTokenAccountInstruction, getAssociatedTokenAddressSync, createTransferInstruction, } from '@solana/spl-token';
import { PublicKey, SystemProgram, clusterApiUrl, Connection, SYSVAR_RENT_PUBKEY, SYSVAR_CLOCK_PUBKEY, sendAndConfirmTransaction, Transaction, ComputeBudgetProgram, } from '@solana/web3.js';
...
import { keypairIdentity, generateSigner, transactionBuilder, signerIdentity, createSignerFromKeypair, base58, } from '@metaplex-foundation/umi';
import { createUmi } from '@metaplex-foundation/umi-bundle-defaults';
import { mplTokenMetadata, verifyCollectionV1, fetchMetadataFromSeeds, createFungibleAsset, createNft, fetchDigitalAsset, } from '@metaplex-foundation/mpl-token-metadata';
...
import { getWallet, pda, sleep } from '../utils.js';
...
export class SolanaManager {
    ...
    getMetadataPDA(mint) {
        ...
        return pda([utf8.encode('metadata'), metaplexMetadata.toBuffer(), mint.toBuffer()], metaplexMetadata);
    }
    /**
    ...
    async loadNosanaJobs() {
        if (!this.jobs) {
            ...
            this.jobs = new Program(idl, programId);
        }
    }
    /**
    ...
    async loadNosanaNodes() {
        if (!this.nodes) {
            ...
            const idl = (await Program.fetchIdl(programId.toString()));
            this.nodes = new Program(idl, programId);
        }
    }
    /**
    ...
    async loadNosanaStake() {
        if (!this.stake || !this.stake.program) {
            ...
            const idl = (await Program.fetchIdl(programId.toString()));
            const poolIdl = (await Program.fetchIdl(poolProgramId.toString()));
            const rewardIdl = (await Program.fetchIdl(rewardsProgramId.toString()));
            this.stake.program = new Program(idl, programId);
    ...
    async setStakeAccounts() {
        if (!this.stakeAccounts) {
            ...
            const mint = new PublicKey(this.config.nos_address);
            const [vault] = await PublicKey.findProgramAddress([utf8.encode('vault'), mint.toBuffer(), authority.toBuffer()], new PublicKey(this.config.stake_address));
            const [reward] = await PublicKey.findProgramAddress([utf8.encode('reward'), authority.toBuffer()], rewardsProgramId);
            const [reflection] = await PublicKey.findProgramAddress([utf8.encode('reflection')], rewardsProgramId);
            const [stake] = await PublicKey.findProgramAddress([utf8.encode('stake'), mint.toBuffer(), authority.toBuffer()], new PublicKey(this.config.stake_address));
            this.stakeAccounts = {
                // solana native
                systemProgram: SystemProgram.programId,
            ...
            const [poolVault] = await PublicKey.findProgramAddress([utf8.encode('vault'), poolId.toBuffer()], poolsProgramId);
    ...
    async createNodeCollection() {
        const umi = createUmi(this.config.network, {
            ...
            .use(mplToolbox());
        const signer = createSignerFromKeypair(umi, fromWeb3JsKeypair((this.provider?.wallet).payer));
        ...
        }).prepend(setComputeUnitPrice(umi, {
    ...
    async createAndVerifySft(collectionAddress) {
        ...
        const signer = createSignerFromKeypair(umi, fromWeb3JsKeypair((this.provider?.wallet).payer));
        umi.use(signerIdentity(signer));
        const umiKeypair = keypairIdentity(fromWeb3JsKeypair((this.provider?.wallet).payer));
        umi.use(umiKeypair);
