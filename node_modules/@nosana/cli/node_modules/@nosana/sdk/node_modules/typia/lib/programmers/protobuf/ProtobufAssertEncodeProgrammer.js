...
(function (ProtobufAssertEncodeProgrammer) {
    ProtobufAssertEncodeProgrammer.decompose = function (props) {
        var assert = AssertProgrammer_1.AssertProgrammer.decompose(__assign(__assign({}, props), { project: __assign(__assign({}, props.project), { options: __assign(__assign({}, props.project.options), { functional: false, numeric: true }) }), equals: false, guard: false }));
        var encode = ProtobufEncodeProgrammer_1.ProtobufEncodeProgrammer.decompose(props);
        return {
            functions: __assign(__assign({}, assert.functions), encode.functions),
            statements: __spreadArray(__spreadArray(__spreadArray([], __read(assert.statements), false), __read(encode.statements), false), [
    ...
    ProtobufAssertEncodeProgrammer.write = function (project) {
        return function (modulo) {
            return function (type, name, init) {
                ...
                var result = ProtobufAssertEncodeProgrammer.decompose({
                    project: project,
                    modulo: modulo,
                    importer: importer,
                    type: type,
