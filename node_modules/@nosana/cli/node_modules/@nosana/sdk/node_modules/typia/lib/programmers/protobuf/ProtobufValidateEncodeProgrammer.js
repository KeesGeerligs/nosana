...
var ProtobufEncodeProgrammer_1 = require("./ProtobufEncodeProgrammer");
var ProtobufValidateEncodeProgrammer;
(function (ProtobufValidateEncodeProgrammer) {
    ProtobufValidateEncodeProgrammer.decompose = function (props) {
        var _a;
        var validate = ValidateProgrammer_1.ValidateProgrammer.decompose(__assign(__assign({}, props), { project: __assign(__assign({}, props.project), { options: __assign(__assign({}, props.project.options), { functional: false, numeric: true }) }), equals: false }));
        var encode = ProtobufEncodeProgrammer_1.ProtobufEncodeProgrammer.decompose(props);
        return {
            functions: __assign(__assign({}, validate.functions), encode.functions),
            statements: __spreadArray(__spreadArray(__spreadArray([], __read(validate.statements), false), __read(encode.statements), false), [
                StatementFactory_1.StatementFactory.constant("__validate", validate.arrow),
                StatementFactory_1.StatementFactory.constant("__encode", encode.arrow),
            ], false),
    ...
    ProtobufValidateEncodeProgrammer.write = function (project) {
        return function (modulo) {
            return function (type, name) {
                var importer = new FunctionImporter_1.FunctionImporter(modulo.getText());
                var result = ProtobufValidateEncodeProgrammer.decompose({
                    project: project,
                    modulo: modulo,
                    importer: importer,
                    type: type,
