var OpenApiV3_1Converter;

(function(OpenApiV3_1Converter) {
    OpenApiV3_1Converter.convert = input => {
        if (input["x-samchon-emended"] === true) return input;
        return {
            ...input,
    ...
    const convertComponents = input => ({
        schemas: input.schemas ? Object.fromEntries(Object.entries(input.schemas).filter((([_, v]) => v !== undefined)).map((([key, value]) => [ key, convertSchema(input)(value) ]))) : undefined,
    ...
    const convertSchema = components => input => {
        ...
        const visit = schema => {
            ...
            if (TypeChecker.isMixed(schema)) {
            ...
            } else if (TypeChecker.isOneOf(schema)) schema.oneOf.forEach(visit); else if (TypeChecker.isAnyOf(schema)) schema.anyOf.forEach(visit); else if (TypeChecker.isAllOf(schema)) union.push(convertAllOfSchema(components)(schema)); else if (TypeChecker.isBoolean(schema)) if (schema.enum?.length && schema.enum.filter((e => e !== null)).length) for (const value of schema.enum.filter((e => e !== null))) union.push({
                const: value,
                ...{
                    ...schema,
                    type: undefined,
                ...
                ...schema,
                ...{
                    properties: schema.properties ? Object.fromEntries(Object.entries(schema.properties).filter((([_, v]) => v !== undefined)).map((([key, value]) => [ key, convertSchema(components)(value) ]))) : undefined,
...
})(OpenApiV3_1Converter || (OpenApiV3_1Converter = {}));

export { OpenApiV3_1Converter };
//# sourceMappingURL=OpenApiV3_1Converter.mjs.map
