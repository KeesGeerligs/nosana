...
(function(MigrateRouteAccessor) {
    MigrateRouteAccessor.overwrite = routes => {
        const dict = collect((op => op.emendedPath.split("/").filter((str => !!str.length && str[0] !== ":")).map(StringUtil.normalize).map((str => Escaper.variable(str) ? str : `_${str}`))))(routes);
        for (const props of dict.values()) props.entries.forEach(((entry, i) => {
            entry.alias = StringUtil.escapeDuplicate([ ...props.children, ...props.entries.filter(((_, j) => i !== j)).map((e => e.alias)) ].map(StringUtil.normalize))(StringUtil.normalize(entry.alias));
            ...
            parameters.forEach(((p, i) => p.key = StringUtil.escapeDuplicate([ "connection", entry.alias, ...parameters.filter(((_, j) => i !== j)).map((y => y.key)) ])(p.key)));
        }));
    };
    ...
    const getName = op => {
        ...
        return method + "By" + op.parameters.map((p => StringUtil.capitalize(p.key))).join("And");
    };
