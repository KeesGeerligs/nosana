...
(function(MigrateRouteConverter) {
    MigrateRouteConverter.convert = props => {
        ...
        const [headers, query] = [ "header", "query" ].map((type => {
            ...
            const primitives = parameters.filter((p => OpenApiTypeChecker.isBoolean(p.schema) || OpenApiTypeChecker.isInteger(p.schema) || OpenApiTypeChecker.isNumber(p.schema) || OpenApiTypeChecker.isString(p.schema) || OpenApiTypeChecker.isArray(p.schema)));
            const out = elem => ({
                ...elem,
                name: type,
                key: type,
            ...
            const entire = [ ...objects.map((o => OpenApiTypeChecker.isObject(o) ? o : props.document.components.schemas?.[o.$ref.replace(`#/components/schemas/`, ``)])), {
                ...
                required: [ ...primitives.filter((p => p.required)).map((p => p.name)), ...dto ? dto.required ?? [] : [] ]
            } ];
            return parameters.length === 0 ? null : out({
                schema: emplaceReference({
                    document: props.document,
                    name: StringUtil.pascal(`I/Api/${props.path}`) + "." + StringUtil.pascal(`${props.method}/${type}`),
                    schema: {
                        type: "object",
        ...
        const parameterNames = StringUtil.splitWithNormalization(props.emendedPath).filter((str => str[0] === ":")).map((str => str.substring(1)));
        ...
        const parameters = (props.operation.parameters ?? []).filter((p => p.in === "path")).map(((p, i) => ({
            ...
            key: (() => {
                let key = StringUtil.normalize(parameterNames[i]);
    ...
    const writeRouteComment = props => {
        ...
        if (props.operation.summary) {
            const emended = props.operation.summary.endsWith(".") ? props.operation.summary : props.operation.summary + ".";
            if (!!description.length && !description.startsWith(props.operation.summary)) description = `${emended}\n${description}`;
        }
...
})(MigrateRouteConverter || (MigrateRouteConverter = {}));

export { MigrateRouteConverter };
//# sourceMappingURL=MigrateRouteConverter.mjs.map
