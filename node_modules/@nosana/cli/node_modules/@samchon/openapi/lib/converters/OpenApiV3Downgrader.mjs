...
(function(OpenApiV3Downgrader) {
    ...
    OpenApiV3Downgrader.downgradeSchema = collection => input => {
        ...
        visit(input);
        visitConstant(input);
        if (nullable === true) for (const u of union) if (OpenApiTypeChecker.isReference(u)) downgradeNullableReference(collection)(u); else u.nullable = true;
    ...
    const downgradeNullableReference = collection => schema => {
        const key = schema.$ref.split("/").pop();
        if (key.endsWith(".Nullable")) return;
        const found = collection.original.schemas?.[key];
        if (found === undefined) return; else if (collection.downgraded.schemas[`${key}.Nullable`] === undefined) {
            collection.downgraded.schemas[`${key}.Nullable`] = {};
            collection.downgraded.schemas[`${key}.Nullable`] = OpenApiV3Downgrader.downgradeSchema(collection)(found);
        }
        schema.$ref += ".Nullable";
    };
