...
(function(SwaggerV2Downgrader) {
    ...
    SwaggerV2Downgrader.downgradeSchema = collection => input => {
        ...
        visit(input);
        if (nullable) for (const u of union) if (OpenApiTypeChecker.isReference(u)) downgradeNullableReference(collection)(u); else u["x-nullable"] = true;
    ...
    const downgradeNullableReference = collection => schema => {
        ...
        if (found === undefined) return; else if (collection.downgraded[`${key}.Nullable`] === undefined) {
            collection.downgraded[`${key}.Nullable`] = {};
            collection.downgraded[`${key}.Nullable`] = SwaggerV2Downgrader.downgradeSchema(collection)(found);
        }
        schema.$ref += ".Nullable";
    };
