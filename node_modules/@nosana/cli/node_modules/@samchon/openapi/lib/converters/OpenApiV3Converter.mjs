...
(function(OpenApiV3Converter) {
    OpenApiV3Converter.convert = input => ({
        ...input,
        components: OpenApiV3Converter.convertComponents(input.components ?? {}),
    ...
    const convertParameter = components => input => ({
        ...input,
        schema: OpenApiV3Converter.convertSchema(components)(input.schema),
    ...
    const convertContent = components => record => Object.fromEntries(Object.entries(record).filter((([_, v]) => v !== undefined)).map((([key, value]) => [ key, {
        ...value,
        schema: value.schema ? OpenApiV3Converter.convertSchema(components)(value.schema) : undefined,
    ...
    OpenApiV3Converter.convertComponents = input => ({
        schemas: input.schemas ? Object.fromEntries(Object.entries(input.schemas).filter((([_, v]) => v !== undefined)).map((([key, value]) => [ key, OpenApiV3Converter.convertSchema(input)(value) ]))) : undefined,
    ...
    OpenApiV3Converter.convertSchema = components => input => {
        ...
        const visit = schema => {
            ...
            if (TypeChecker.isAnyOf(schema)) schema.anyOf.forEach(visit); else if (TypeChecker.isOneOf(schema)) schema.oneOf.forEach(visit); else if (TypeChecker.isAllOf(schema)) union.push(convertAllOfSchema(components)(schema)); else if (TypeChecker.isBoolean(schema) || TypeChecker.isInteger(schema) || TypeChecker.isNumber(schema) || TypeChecker.isString(schema)) if (schema.enum?.length && schema.enum.filter((e => e !== null)).length) union.push(...schema.enum.filter((v => v !== null)).map((value => ({
                ...
                ...schema,
                items: OpenApiV3Converter.convertSchema(components)(schema.items)
            }); else if (TypeChecker.isObject(schema)) union.push({
                ...schema,
                ...{
                    properties: schema.properties ? Object.fromEntries(Object.entries(schema.properties).filter((([_, v]) => v !== undefined)).map((([key, value]) => [ key, OpenApiV3Converter.convertSchema(components)(value) ]))) : undefined,
                    additionalProperties: schema.additionalProperties ? typeof schema.additionalProperties === "object" && schema.additionalProperties !== null ? OpenApiV3Converter.convertSchema(components)(schema.additionalProperties) : schema.additionalProperties : undefined
                }
    ...
    const convertAllOfSchema = components => input => {
        ...
        return {
            ...input,
            type: "object",
            properties: Object.fromEntries(objects.map((o => Object.entries(o?.properties ?? {}))).flat().map((([key, value]) => [ key, OpenApiV3Converter.convertSchema(components)(value) ]))),
            ...{
                allOf: undefined
            }
        };
...
})(OpenApiV3Converter || (OpenApiV3Converter = {}));

export { OpenApiV3Converter };
//# sourceMappingURL=OpenApiV3Converter.mjs.map
