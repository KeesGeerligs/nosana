...
const _Propagate = async (from, props) => {
    ...
    const path = props.connection.host[props.connection.host.length - 1] !== "/" && props.route.path[0] !== "/" ? `/${getPath(props)}` : getPath(props);
    const url = new URL(`${props.connection.host}/${path}`);
    ...
    if (status === 200 || status === 201) {
        if (props.route.method === "head") return out(undefined); else if (props.route.success === null || props.route.success.type === "text/plain") return out(await response.text()); else if (props.route.success.type === "application/json") {
