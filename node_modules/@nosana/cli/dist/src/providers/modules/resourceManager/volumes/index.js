 ...
 * @returns
 */
export function createVolumeManager(db, docker, logger) {
    let fetched = false;
    let market_required_volumes = [];
    /**
     * Fetch market required volumes
     * @param market
     */
    const pullMarketRequiredVolumes = async (remoteResources) => {
    ...
    const resyncResourcesDB = async () => {
        ...
        for (const [resource, value] of Object.entries(db.data.resources.volumes)) {
            ...
            if (resource.includes('s3://nos-ai-models-qllsn32u')) {
                db.data.resources.volumes[`${resource.replace('s3://nos-ai-models-qllsn32u', nosanaBucket)}`] = value;
                delete db.data.resources.volumes[resource];
            }
        }
        db.write();
        for (const [resource, { volume, lastUsed, required }] of Object.entries(db.data.resources.volumes)) {
            if (!hasDockerVolume(volume, savedVolumes)) {
                delete db.data.resources.volumes[resource];
                continue;
            }
            if ((!fetched && required) ||
                market_required_volumes.some((vol) => createResourceName(vol) === resource))
                continue;
            ...
            if (hoursSinceLastUsed > 24) {
                try {
                    ...
                    logger.log(chalk.red(`Could not remove remote resource: ${resource}.\n${message}`));
                }
            }
        }
        db.write();
     ...
     * @param s3Bucket
     */
    const createRemoteVolume = async (resource) => {
        ...
        logger.log(chalk.cyan(`Fetching remote resource ${chalk.bold(resourceName)}`), true);
        try {
     ...
     */
    const hasVolume = async (resource) => {
        let result;
     ...
     * @param volume
     */
    const setVolume = (bucket, volume) => {
        db.data.resources.volumes[bucket] = {
            volume,
            required: market_required_volumes.some((vol) => createResourceName(vol) === bucket),
            lastUsed: new Date(),
    ...
    const pruneVolumes = async () => {
        ...
        for (const { Name } of cachedVolumes) {
            ...
            if (dbEntry)
                delete db.data.resources.volumes[dbEntry[0]];
        }
        db.write();
    };
    return {
