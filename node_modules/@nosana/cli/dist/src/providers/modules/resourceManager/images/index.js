 ...
 * @returns
 */
export function createImageManager(db, docker, logger) {
    let fetched = false;
    let market_required_images = [];
    const pullMarketRequiredImages = async (required_images) => {
        fetched = true;
        ...
        for (const image of market_required_images) {
            ...
            if (!db.data.resources.images[image])
                setImage(image);
        }
    };
    /**
     ...
     * @returns Promise
     */
    const resyncImagesDB = async () => {
        for (const [image, { lastUsed, required }] of Object.entries(db.data.resources.images)) {
            if (!(await docker.hasImage(image))) {
            ...
            if (hoursSinceLastUsed > 24) {
                try {
                    ...
                    throw new Error(`Could not remove removeDanglingImages image: ${image}.\n${err}`);
                }
            }
        }
        db.write();
    ...
    const pruneImages = async () => {
        ...
        for (const { Id, RepoTags } of cachedImages) {
            const dbEntry = Object.entries(db.data.resources.images).find((img) => repoTagsContainsImage(img[0], RepoTags) ? img : undefined);
            ...
            if (dbEntry)
                delete db.data.resources.images[dbEntry[0]];
        }
        db.write();
    };
    return {
