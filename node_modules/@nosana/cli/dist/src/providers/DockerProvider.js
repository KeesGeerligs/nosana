...
export class DockerProvider extends BasicProvider {
    constructor(server, configLocation, gpu, logger) {
        ...
        this.resourceManager = createResourceManager(this.db, this.docker, this.logger);
        /**
         * Run operation and return results
         * @param op Operation specs
         * @returns OpState
         */
        this.supportedOps['container/run'] = async (op, flowId, updateOpState, operationResults) => {
            const flow = this.getFlow(flowId);
            const opStateIndex = flow.state.opStates.findIndex((opState) => op.id === opState.operationId);
            ...
            if (opState.providerId) {
                try {
                    ...
                    container = this.docker.getContainer(opState.providerId);
                    try {
            ...
            if (!opState.providerId) {
                ...
                try {
                    ...
                    const err = new Error(chalk.red(`Cannot pull image ${op.args.image}: `) + error);
                ...
                if (op.args.expose) {
                    ...
                    try {
                        ...
                        const err = new Error(chalk.red(`Cannot pull image ${FRPC_IMAGE}: `) + error);
                        jobDispatch(JOB_STATE_NAME.PULLING_IMAGE, {
                            image: FRPC_IMAGE,
                            error: err,
                        });
                ...
                if (op.args.resources) {
                    ...
                    try {
                        ...
                        const err = new Error(chalk.red(`Cannot pull image ${s3HelperImage}: `) + error);
                        jobDispatch(JOB_STATE_NAME.PULLING_IMAGE, {
                            image: s3HelperImage,
                            error: err,
                        });
                    ...
                    for (const resource of op.args.resources) {
                        ...
                        try {
                            ...
                            const error = new Error(chalk.red(`Cannot pull remote resource ${createResourceName(resource)}:\n`) + err);
                ...
                container = await this.runOpContainerRun(op.args, flowId, opStateIndex, updateOpState);
            }
            if (container) {
         ...
         * @param op Operation specs
         */
        this.supportedOps['container/create-volume'] = async (op, flowId, updateOpState) => {
            const flow = this.getFlow(flowId);
            const opStateIndex = flow.state.opStates.findIndex((opState) => op.id === opState.operationId);
            const opState = flow.state.opStates[opStateIndex];
            this.logger.log(chalk.cyan(`- Creating volume ${chalk.bold(op.args.name)}`), true);
            updateOpState({
                startTime: Date.now(),
                status: 'running',
            });
     ...
     * @returns
     */
    async runOpContainerRun(opArgs, flowId, opStateIndex, updateOpState) {
        const flow = this.getFlow(flowId);
        ...
        if (opArgs.resources) {
            for (const resource of opArgs.resources) {
                if ((await this.resourceManager.volumes.hasVolume(resource)) === false) {
        ...
        const work_dir = opArgs.work_dir ||
            !flow.jobDefinition.global ||
            !flow.jobDefinition.global.work_dir
            ? opArgs.work_dir
            : flow.jobDefinition.global.work_dir;
        const entrypoint = opArgs.entrypoint ||
            !flow.jobDefinition.global ||
            !flow.jobDefinition.global.entrypoint
            ? opArgs.entrypoint
            : flow.jobDefinition.global.entrypoint;
        const globalEnv = flow.jobDefinition.global && flow.jobDefinition.global.env
        ...
        const container = await this.runContainer(opArgs.image ? opArgs.image : flow.jobDefinition.global?.image, {
            name,
            cmd: parsedCmd,
            env: {
                ...globalEnv,
        ...
        if (opArgs.expose) {
            ...
            if (!opArgs.private) {
                this.logger.log(chalk.cyan(`Exposing service at ${chalk.bold(`https://${prefix}.${config.frp.serverAddr}`)}`));
            }
            else {
    ...
    async runContainer(image, { name, networks, cmd, gpu, volumes, env, work_dir, network_mode, entrypoint, restart_policy, }) {
        const devices = gpu
            ? [
                {
                    ...(this.gpu === 'all'
    ...
    async stopFlowOperation(flowId, op) {
        const opState = this.db.data.flows[flowId].state.opStates.find((opState) => opState.operationId === op.id);
        if (!opState)
            throw new Error('could not find opState');
        if (opState.providerId) {
     ...
     */
    async finishOpContainerRun({ container, containerInfo, operationResults, updateOpState, }) {
        ...
        const { logs, results } = extractLogsAndResultsFromLogBuffer(logBuffer, operationResults);
        const updatedOpState = {
            logs,
        ...
        if (results)
            updatedOpState['results'] = results;
        updateOpState(updatedOpState);
    }
    async finishFlow(flowId, status) {
        const flow = this.getFlow(flowId);
        ...
        for (let i = 0; i < flow?.state.opStates.length; i++) {
            ...
            if (flow.state.opStates[i].providerId) {
                await this.stopAndRemoveContainer(flow.state.opStates[i].providerId);
                const frpcName = 'frpc-' + flowId + '-' + flow.state.opStates[i].operationId;
    ...
    async updateMarketRequiredResources(market) {
