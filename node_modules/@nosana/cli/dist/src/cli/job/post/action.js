...
export async function run(command, options) {
    ...
    if (command.length && options.file) {
        console.error(`${colors.YELLOW}WARNING: [command] ignored as file flag is already set${colors.RESET}`);
    }
    const nosana = getSDK();
    let json_flow; // TODO: add JSON flow type
    if (options.file) {
    ...
    try {
        ...
        if (new PublicKey(nosana.solana.config.market_address) &&
    ...
    if (type === 'slug') {
        try {
            const { data: marketResponse, error } = await clientSelector().GET('/api/markets/{id}/', {
            ...
            return formatter.throw(OUTPUT_EVENTS.OUTPUT_FAILED_TO_FETCH_MARKETS_ERROR, { error: error });
        }
    }
    const market = await nosana.jobs.getMarket(nosana.solana.config.market_address);
    ...
    await sleep(3);
    if (isExposed(json_flow)) {
        if (!isPrivate(json_flow)) {
