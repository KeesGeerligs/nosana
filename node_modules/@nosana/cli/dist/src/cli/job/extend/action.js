...
export async function extendJob(jobAddress, options) {
    const nosana = getSDK();
    ...
    if (job) {
        formatter.output(OUTPUT_EVENTS.OUTPUT_JOB_URL, {
            job_url: `https://dashboard.nosana.com/jobs/${jobAddress}${nosana.solana.config.network.includes('devnet') ? '?network=devnet' : ''}`,
        });
