...
export async function getJob(jobAddress, options, cmd) {
    const nosana = getSDK();
    ...
    if (job) {
        formatter.output(OUTPUT_EVENTS.OUTPUT_JOB_URL, {
            job_url: `https://explorer.nosana.io/jobs/${jobAddress}${nosana.solana.config.network.includes('devnet') ? '?network=devnet' : ''}`,
        });
        ...
        formatter.output(OUTPUT_EVENTS.OUTPUT_MARKET_URL, {
            market_url: `https://explorer.nosana.io/markets/${job.market}${nosana.solana.config.network.includes('devnet') ? '?network=devnet' : ''}`,
        });
        ...
        if ((options.wait || options.download) &&
            ...
            if (job.state === 'RUNNING') {
                ...
                formatter.output(OUTPUT_EVENTS.OUTPUT_NODE_URL, {
                    url: `https://explorer.nosana.io/nodes/${job.node}${nosana.solana.config.network.includes('devnet')
                        ? '?network=devnet'
                        : ''}`,
                });
                ...
                ws = listenToWebSocketLogs(`https://${job.node}.${configs(options).frp.serverAddr}`, jobAddress);
        ...
        if (job.state === 'COMPLETED') {
            ...
            if (ipfsResult.opStates) {
                ...
                for (let i = 0; i < ipfsResult.opStates.length; i++) {
                    const opState = ipfsResult.opStates[i];
                        ...
                        if (Array.isArray(steps)) {
                            ...
                            if (logs && logs[logs.length - 2]) {
                                ...
                                if (options.download) {
                                    ...
                                    console.log(`${colors.YELLOW}This job has external artifacts that can be downloaded with:${colors.RESET}`);
                                    console.log(`${colors.CYAN}nosana download ${ipfshash}${colors.RESET}`);
                                }
                            }
                        }
                    }
...
async function fetchServiceURLWithRetry(job, jobAddress, formatter, headers) {
    const retryInterval = 5000; // 5 seconds
    const intervalId = setInterval(async () => {
        try {
            const response = await fetch(`https://${job.node}.${configs().frp.serverAddr}/service/url/${jobAddress}`, { method: 'GET', headers });
