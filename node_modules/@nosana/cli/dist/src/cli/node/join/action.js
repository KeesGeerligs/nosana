...
let flow;
let provider;
export async function runBenchmark(options, shouldKillProgram = true) {
    const nosana = getSDK();
    ...
    switch (options.provider) {
        case 'podman':
            provider = new PodmanProvider(options.podman, options.config, options.gpu);
            break;
        case 'docker':
        default:
            provider = new DockerProvider(options.podman, options.config, options.gpu);
            break;
    }
    ...
    if (!validation.success) {
        ...
        answers = {
            ...
            discord: await input({
                message: "What is your Discord username? (If you don't use Discord, leave blank)",
            }),
            twitter: await input({
        ...
        result = await provider.waitForFlowFinish(flow.id, (log) => {
            if (log.type === 'info') {
                console.log(log.log);
            }
    ...
    if (result && result.status === 'success' && result.opStates && answers) {
        try {
            ...
            const signature = (await nosana.solana.signMessage(conf.signMessage));
            ...
            console.log('================================');
            console.log(chalk.green('Thank you for registering your Nosana Node. We will selectively onboard new participants into the Nosana Grid based on market requirements and ability of your hardware to run advanced AI models.'));
        ...
        throw new Error(chalk.red(`Couldn't succesfully run benchmark, finished with status: ${result ? result.status : 'cleared'}`));
    }
}
