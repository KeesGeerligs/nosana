...
export async function migrateWalletCommand(walletPath, startup = false) {
    if (!startup) {
    ...
    const suspectedKeyPair = nosana.solana.wallet.payer;
    const { isCompromised, isAtRisk, reimbursementTransaction, newNodeAddress } = await validatePublicKey(suspectedKeyPair.publicKey);
    if (!isAtRisk)
        return false;
    ...
    if (!newNodeAddress) {
        ...
        console.log(chalk.yellow(`${heading}. This process will involve:
    ...
    - Onboard your new wallet for nosana grid.`));
        ...
        console.log(chalk.yellow(`Transfered possible NOS tokens and SOL, any remaining will need to be manually transfered.`));
    }
    else {
        return false;
    }
