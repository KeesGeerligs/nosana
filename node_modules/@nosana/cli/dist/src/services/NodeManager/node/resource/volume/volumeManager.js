...
import { createS3HelperOpts, nosanaBucket, s3HelperImage, } from '../../../../../providers/modules/resourceManager/volumes/definition/s3HelperOpts.js';
...
export class VolumeManager {
    ...
    async createRemoteVolume(resource) {
    ...
    async runResourceManagerContainer(name, resource) {
        const { url, files } = resource;
        const args = createS3HelperOpts(name, { url, files }, resource.IAM);
        ...
        logStream.on('data', (logBuffer) => {
            try {
                ...
                if (!start && logJSON.event === 'status') {
                    ...
                    formatSize = format;
                    this.progressBarReporter.start(`downloading resource volume resource ${name}`, {
        ...
        if (StatusCode !== 0) {
            this.progressBarReporter.stop(`downloading resource volume resource ${name} stopped`);
        ...
        this.progressBarReporter.stop(`downloading resource volume resource ${name} completed`);
