...
export class Provider {
    constructor(containerOrchestration, repository, resourceManager) {
    ...
    async setUpReverseProxyApi(address) {
        ...
        try {
            ...
            const tunnel_name = `tunnel-api-${address}`;
            const frpc_name = `frpc-api-${address}`;
            let { status, error } = await this.containerOrchestration.createNetwork(networkName);
            if (!status) {
                throw error;
            }
            const networks = {};
            networks[networkName] = {};
            ({ status, error } = await this.containerOrchestration.pullImage(frpcImage));
            if (!status) {
                throw error;
            }
            ({ status, error } = await this.containerOrchestration.pullImage(tunnelImage));
            if (!status) {
                throw error;
            }
            ({ status, error, result } =
    ...
    async startServiceExposedUrlHealthCheck(id, container, port) {
    ...
    stopServiceExposedUrlHealthCheck() {
        if (this.exposedUrlHealthCheck) {
            clearInterval(this.exposedUrlHealthCheck);
            this.exposedUrlHealthCheck = undefined;
        }
    }
    async containerRunOperation(id, index) {
        const frpcImage = 'registry.hub.docker.com/nosana/frpc:0.1.0';
        const flow = this.repository.getflow(id);
        const opState = this.repository.getOpState(id, index);
        const op = flow.jobDefinition.ops[index];
        try {
            this.repository.updateOpState(id, index, {
                startTime: Date.now(),
                status: 'running',
            });
            let container = await this.containerOrchestration.getContainer(opState.providerId);
            const exist = (await this.containerOrchestration.doesContainerExist(opState.providerId)).result;
            const exited = (await this.containerOrchestration.isContainerExited(opState.providerId)).result;
            if (exist && !exited && container.id) {
                ...
                let result;
                let { status, error } = await this.containerOrchestration.pullImage(op.args.image);
                if (!status) {
                    throw error;
                }
                ...
                if (op.args.expose) {
                    networks[name] = {};
                    ({ status, error } = await this.containerOrchestration.createNetwork(name));
                    if (!status) {
                        throw error;
                    }
                ...
                if (op.args.resources) {
                    ({ status, error } = await this.containerOrchestration.pullImage(s3HelperImage));
                    if (!status) {
                        throw error;
                    }
                ...
                ({ status, error, result } =
                    await this.containerOrchestration.runFlowContainer(op.args.image ?? flow.jobDefinition.global?.image, {
                        name,
                        cmd,
                        env,
                        networks,
                ...
                const logBuffer = await promiseTimeoutWrapper(container.logs({
                    stdout: true,
                    stderr: true,
                    follow: false,
                    abortSignal: controller.signal,
                }), 360, controller);
                const { logs, results } = extractLogsAndResultsFromLogBuffer(logBuffer, op.results);
                this.repository.updateOpState(id, index, {
                    logs,
                    results,
    ...
    async containerRunStopOperation(id, index) {
        const flow = this.repository.getflow(id);
        const opState = this.repository.getOpState(id, index);
        const name = flow.id + '-' + opState.operationId;
    ...
    async volumeCreateOperation(id, index) {
        const flow = this.repository.getflow(id);
        const op = flow.jobDefinition.ops[index];
        try {
            this.repository.updateOpState(id, index, {
            ...
            let { status, result, error } = await this.containerOrchestration.getVolume(name);
            if (!status) {
                let { status, result, error } = await this.containerOrchestration.createVolume(name);
                if (!status) {
                    throw error;
                }
                this.repository.updateOpState(id, index, {
    ...
    async volumeStopOperation(id, index) {
        const flow = this.repository.getflow(id);
        const opState = this.repository.getOpState(id, index);
        const name = flow.id + '-' + opState.operationId;
        let { status, result, error } = await this.containerOrchestration.deleteVolume(name);
        return true;
    }
    runOperation(type, param) {
...
function getEntrypoint(arg, flow) {
...
function getGpu(arg, flow) {
    return (arg.gpu || (flow.jobDefinition.global && flow.jobDefinition.global.gpu));
}
function getVolumes(arg, flow) {
    const volumes = [];
