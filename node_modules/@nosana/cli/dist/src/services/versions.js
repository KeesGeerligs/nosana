...
function requiresNewVersion(required, current) {
    return parseInt(required) > parseInt(current);
}
function exitCleanly(currentVersion, latestVersion) {
    console.log(chalk.red(`You are currently running Nosana CLI version ${currentVersion}. Version ${latestVersion} has been released, please update your CLI to the latest version using npm install @nosana/cli.`));
    process.exit(129);
}
export async function validateCLIVersion() {
    try {
        const [current_major, current_minor, current_patch] = pkg.version.split('.');
        if (parseInt(current_major) === 0 &&
        ...
        const [required_major, required_minor, required_patch] = registryLatestVersion.split('.');
        ...
        throw new Error(`${chalk.red("Failed to fetch CLI's minium required version.")}\n${error}`);
    }
}
