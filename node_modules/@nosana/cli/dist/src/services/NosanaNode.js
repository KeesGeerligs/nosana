...
import { askYesNoQuestion, isNodeOnboarded, SECONDS_PER_DAY, sleep, } from '../generic/utils.js';
...
export class NosanaNode {
    constructor(client, providerName = 'podman', providerUrl = 'http://localhost:8080', configLocation = '~/.nosana/', gpu) {
        this.market = undefined;
        this.run = undefined;
        this.logger = new Logger();
        switch (providerName) {
            case 'podman':
                this.provider = new PodmanProvider(providerUrl, configLocation, gpu, this.logger);
                break;
            case 'docker':
            default:
                this.provider = new DockerProvider(providerUrl, configLocation, gpu, this.logger);
                break;
        }
        this.sdk = client;
        this.address = this.sdk.solana.provider.wallet.publicKey.toString();
    }
    async ensureContainerDoesNotExist(containerName) {
    ...
    async startAPI() {
        ...
        this.logger.succeed(chalk.cyan(`Node API running at ${chalk.bold(tunnelServer)}`));
        return port;
    }
    async checkRun() {
    ...
    async waitForJob(marketAccount) {
        return new Promise(async (resolve, reject) => {
            ...
            const expireInterval = setInterval(async () => {
                ...
                if (flow) {
                    const isFlowExposed = flow.jobDefinition.ops.filter((op) => op.type === 'container/run' &&
                        op.args.expose).length > 0;
                    if (isFlowExposed) {
                        // Finish the job if the job timeout has been reached and we expose a service
                        if (NosanaNode.isRunExpired(this.run, 
                        ...
                        if (NosanaNode.isRunExpired(this.run, marketAccount.jobTimeout * 1.5)) {
                            // JOB_EXPIRED
                            // STOPING_JOB
                            clearInterval(expireInterval);
            ...
            try {
                ...
                const flowResult = await this.provider.waitForFlowFinish(this.run.account.job.toString());
                //FLOW_FINISHED
                this.logger.succeed();
                clearInterval(expireInterval);
                resolve(flowResult);
    ...
    async checkQueued(market) {
        ...
        for (let i = 0; i < markets.length; i++) {
            ...
            if (market &&
                market.queue &&
                market.queue.find((e) => e.toString() === this.address)) {
                this.market = market;
                return market;
            }
        }
    ...
    async waitForRun(market, enableQueueCheck = false) {
        ...
        return new Promise((resolve, reject) => {
            ...
            subscriptionId = this.sdk.jobs.connection.onProgramAccountChange(jobProgram.programId, async (event) => {
            ...
            if (typeof subscriptionId !== 'undefined')
                this.sdk.jobs.connection.removeProgramAccountChangeListener(subscriptionId);
            if (getRunsInterval)
                clearInterval(getRunsInterval);
            if (checkQueuedInterval)
                clearInterval(checkQueuedInterval);
            return run;
        })
            .catch((error) => {
            if (typeof subscriptionId !== 'undefined')
    ...
    async finishJob(job, run, result) {
        this.logger.log(chalk.cyan('Uploading results to IPFS'), true);
        let ipfsResult;
        try {
            ipfsResult = await this.sdk.ipfs.pin(result);
        ...
        try {
            const tx = await this.sdk.jobs.submitResult(bytesArray, run, job.market.toString());
    ...
    async healthCheck({ market, marketAccount, accessKey, printDetailed = true, }) {
        ...
        try {
            ...
            this.logger.fail('Could not check SOL balance, make sure you have enough SOL');
        }
        if (stats) {
            const solBalance = stats.sol / 1e9;
            if (solBalance < 0.005) {
                this.logger.fail(chalk.red.bold('Not enough SOL balance'));
                const error = new Error(`SOL balance ${solBalance} should be 0.005 or higher. Send some SOL to your node address ${chalk.cyan(this.address)} `);
            ...
            if (printDetailed) {
                this.logger.succeed(chalk.green(`Sol balance: ${chalk.bold(solBalance)}`));
            }
        }
        if (printDetailed) {
        ...
        try {
            ...
            this.logger.fail(chalk.red(`${chalk.bold(this.provider.name)} provider not healthy`));
            throw error;
        }
        if (printDetailed) {
            this.logger.succeed(chalk.green(`${chalk.bold(this.provider.name)} is running on ${chalk.bold(`${this.provider.protocol}://${this.provider.host}:${this.provider.port}`)}`));
        }
        try {
        ...
        if (printDetailed) {
            this.logger.succeed(chalk.green(`Stake found with ${chalk.bold(addressStake.amount / 1e6)} NOS staked with unstake duration of ${chalk.bold(addressStake.duration / SECONDS_PER_DAY)} days`));
        }
        ...
        if (addressStake) {
            ...
            if (minStakeForMarket > 0 && currentStake < minStakeForMarket) {
                ...
                if (!nosBalance ||
                    ...
                    throw new Error(chalk.red(`Cannot enter Market, Insufficient NOS Balance of ${Number(nosBalance?.amount || 0) / 1e6} for Top up of Market minimum Stake of ${minStakeForMarket / 1e6}`));
                }
                ...
                await askYesNoQuestion(chalk.yellow(`Do you want to top up your stake with ${chalk.bold(diff / 1e6)} NOS with unstake duration of ${chalk.bold(addressStake.duration / SECONDS_PER_DAY)} days? (Y/n):`), async () => {
                    try {
                        ...
                        if (Number(addressStake.amount || 0) >= minStakeForMarket) {
                            this.logger.succeed();
                            console.log(chalk.green(`Stake successfully topped up. New stake amount: ${addressStake.amount / 1e6} NOS.`));
                        }
                        else {
                            this.logger.fail();
                            throw new Error(chalk.red(`Transaction not completed yet, possibly still pending. Try again.`));
                        }
                    }
                    catch (topUpError) {
                        this.logger.fail();
                        const error = new Error(chalk.red(`Failed to top up stake: ${topUpError.message}`));
                        nodeDispatch(NODE_STATE_NAME.HEALTH_CHECK_FAILED, {
                            error: error,
                        });
                        throw error;
                    ...
                    const error = new Error(chalk.red(`Cannot enter Market, Insufficient Staked NOS for Market`));
                    nodeDispatch(NODE_STATE_NAME.HEALTH_CHECK_FAILED, {
                        error: error,
                    });
                    throw error;
        ...
        try {
            if (marketAccount.nodeAccessKey.toString() === EMPTY_ADDRESS.toString()) {
                ...
                if (printDetailed) {
                    this.logger.log(chalk.cyan(`Checking required access key for market ${chalk.bold(market)}`), true);
                }
                const accessKeyFromChain = await this.sdk.solana.getNftFromCollection(this.address, marketAccount.nodeAccessKey.toString());
                if (accessKeyFromChain) {
                    accessKey = accessKeyFromChain;
                    if (printDetailed) {
                        this.logger.succeed(chalk.green(`Found access key ${chalk.bold(accessKey)} for market ${chalk.bold(market)}`));
                    }
                }
                else {
                    if (!accessKey) {
                    ...
                    this.logger.succeed(chalk.yellow(`Could not find key on-chain, trying access key ${chalk.bold(accessKey)} for market ${chalk.bold(market)}`));
                }
            }
        }
        catch (e) {
            this.logger.fail(chalk.red(`Denied access to market ${chalk.bold(market)}`));
            nodeDispatch(NODE_STATE_NAME.HEALTH_CHECK_FAILED, {
                error: e,
            });
            throw e;
    ...
    async runBenchmark(printDetailed = true) {
        let gpus;
        try {
            /****************
             * Benchmark *
            ...
            result = await this.provider.waitForFlowFinish(flow.id, (event) => {
                if (printDetailed) {
            ...
            if (result &&
                ...
                const cudaCheckResults = JSON.parse(result.opStates[0].logs[0].log);
                ...
                getNodeStateManager().setSharedData('devices', cudaCheckResults.devices);
                ...
                for (let i = 0; i < result.opStates[1].logs.length; i++) {
                    ...
                    if (ds.log) {
                        ...
                        if (config.minDiskSpace > availableDiskSpace) {
                            const error = new Error(`Not enough disk space available, found ${availableDiskSpace / 1000} GB available. Needs minimal ${config.minDiskSpace / 1000} GB.`);
                            nodeDispatch(NODE_STATE_NAME.BENCHMARK_FAILED, {
                                error: error,
                            });
                            throw error;
                ...
                if (result.opStates[0]) {
                    const cudaCheckResults = JSON.parse(result.opStates[0].logs[0].log);
                    if (cudaCheckResults.error) {
                        output.push(`GPU benchmark failed, please ensure your NVidia Cuda runtime drivers are up to date and your NVidia Container Toolkit is correctly configured.`);
                    }
            ...
            console.error(chalk.red('Something went wrong while detecting hardware', e));
            throw e;
        }
    ...
    async join() {
        ...
        const signature = (await this.sdk.solana.signMessage(config.signMessage));
        ...
        try {
            ...
            const response = await fetch(`${config.backendUrl}/nodes/${this.address}`, {
                method: 'GET',
                headers: {
            ...
            if (e instanceof Error && e.message.includes('Node not onboarded yet')) {
                throw new Error(chalk.yellow('Node is still on the waitlist, wait until you are accepted.'));
            }
            else if (e instanceof Error && e.message.includes('Node not found')) {
                throw new Error(chalk.yellow('Node is not registred yet. To register run the `node join` command.'));
            }
            throw e;
        }
        // benchmark
        ...
        try {
            ...
            const response = await fetch(`${config.backendUrl}/nodes/${this.address}/check-market`, {
                method: 'POST',
                headers: {
                    Authorization: `${this.address}:${base64Signature}`,
                    'Content-Type': 'application/json',
                ...
                if ((data.message &&
                    ...
                    try {
                        ...
                        try {
                            ...
                            const feePayer = (this.sdk.solana.provider?.wallet)
                                .payer;
                            const recoveredTransaction = await getRawTransaction(Uint8Array.from(Object.values(data.tx)));
                            ...
                            this.logger.fail();
                            throw new Error(chalk.red('Couldnt send SFT transaction from backend ') +
                                e.message);
                        }
                    }
                    catch (e) {
                        this.logger.fail();
                        throw new Error(chalk.red('Something went wrong with minting your access key, please try again. ') + e.message);
                    }
    ...
    static isRunExpired(run, expireTime) {
