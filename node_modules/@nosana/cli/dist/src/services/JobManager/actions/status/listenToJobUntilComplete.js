...
export async function listenToJobUntilComplete(jobId, statusEmitter) {
    return new Promise(async (resolve) => {
        ...
        const { node: runNode, state: runState } = await waitForJobRunOrCompletion(id);
        if (runState === 'RUNNING') {
            if (state !== runState) {
        ...
        const { node: finalNode, state: finalState } = await waitForJobCompletion(id);
