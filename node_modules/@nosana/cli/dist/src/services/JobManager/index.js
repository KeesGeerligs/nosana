...
export default class JobManager {
    ...
    async status(id, onEvent, onClose) {
    ...
    async post(market, job, options) {
        const { group_id, recursive, recursive_offset_min } = options;
        if (group_id && this.state.get(group_id)) {
            throw new Error('Group id already exists, please stop the existing group or update the group.');
        }
        let runId;
        const recurisveLoop = async (market, job, options) => {
            const jobResult = await postJobWithOptions(market, job, options, ({ id, active_nodes }) => {
                const timeout = active_nodes[0].job_timeout;
                if (recursive) {
                    this.workers.set(id, setTimeout(() => {
    ...
    async stop(id, waitForRunning = false) {
        await this.nosana.jobs.loadNosanaJobs();
        ...
        if (localGroup) {
            ...
            const failedJobs = [];
            await Promise.all(localGroup.active_nodes.map(async ({ job }) => {
            ...
            this.state.set(id, {
                ...localGroup,
                active_nodes: localGroup.active_nodes.filter(({ job }) => failedJobs.includes(job)),
                expired_nodes: [
