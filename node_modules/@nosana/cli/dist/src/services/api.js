...
const verifyJobOwnerMiddleware = async (req, res, next) => {
    const jobId = req.params.jobId;
    if (!jobId) {
        res.status(400).send('jobId path parameter is required');
        return;
    }
    const job = await node.sdk.jobs.get(jobId);
...
app.get('/service/url/:jobId', verifySignatureMiddleware, (req, res) => {
    try {
        const jobId = req.params.jobId;
...
app.get('/status/:jobId', verifySignatureMiddleware, verifyJobOwnerMiddleware, (req, res) => {
    const jobId = req.params.jobId;
    const logType = req.query.logs;
    if (logType) {
...
app.post('/service/stop/:jobId', verifySignatureMiddleware, verifyJobOwnerMiddleware, async (req, res) => {
    const jobId = req.params.jobId;
    if (!jobId) {
        res.status(400).send('jobId path parameter is required');
        return;
